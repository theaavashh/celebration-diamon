...
const MarqueeBanner = () => {
  const topRowWords = ["Natural Diamonds", "In-house Geomologist", "Premium Quality", "Expert Craftsmanship"];
  const bottomRowWords = ["30% off on all diamonds products", "Dashain Offer", "Celebration Diamond Studio", "Nepal's Finest Jewelry"];
  ...
  return (
    <section className="w-full py-6 bg-gradient-to-r from-amber-500 to-amber-600 relative overflow-hidden mt-10">
      {/* Decorative elements */}
        ...
        >
          {duplicatedTopWords.map((word, index) => (
            ...
            >
              ...
              <Gem className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 lg:w-9 lg:h-9 text-white ml-10" />
            </div>
          ))}
        </motion.div>
      </div>
