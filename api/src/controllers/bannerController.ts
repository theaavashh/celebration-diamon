...
export const listBanners = async (req: Request, res: Response) => {
  try {
    const items = await (prisma as any).banner.findMany({ orderBy: { createdAt: 'desc' }, take: 1 });
    const data = items.map((b: any) => ({ id: b.id, imageUrl: b.imageUrl, isActive: b.isActive, createdAt: b.createdAt }));
